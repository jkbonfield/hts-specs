...
\Procedure{RangeEncode}{$sym\_low, sym\_freq, tot\_freq$}
  ...
  \State \algalign{range}{\gets} $range / tot\_freq$
...
\Procedure{RangeShiftLow}{}
  \If{$low < $ 0xff000000 or $carry \ne 0$}
    ...
    \Else
      ...
      \EndWhile
    \EndIf
    \State $cache \gets low >> 24$ \Comment{Copy of top byte ready for next flush}
    \State $carry \gets 0$
  \Else
    \State $FFnum \gets FFnum + 1$
  \EndIf
...
\Procedure{RangeEncodeEnd}{}
  \For{$i \gets 0$ to $5$} \Comment{Flush any residual state in $low$}
    \State \Call{RangeShiftLow}{}
  \EndFor
\EndProcedure
\end{algorithmic}
...
\Function{ModelDecode}{rc}
  ...
  \If{$x > 0$ and $F_x > F_{x-1}$}
    \State \Call{SwapElement}{$F$, $x$, $x-1$} \Comment{swap $F_x$ with $F_{x-1}$}
    \State \Call{SwapElement}{$S$, $x$, $x-1$} \Comment{swap $S_x$ with $S_{x-1}$}
  \EndIf
  \State \Return $sym$
\EndFunction
\end{algorithmic}
...
\subsubsection*{Order-0 and Order-1 Encoding}
...
\subsubsection*{RLE with Order-0 and Order-1 Encoding}
...
\multicolumn{6}{|l|}{}\\[-0.3em]
\multicolumn{6}{|l|}{\textit{If \textsc{Dict} flag is set (and neither \textsc{X4} or \textsc{Cat} flags are set):} } \\
\cline{2-5}
& ? & uint8[] & dict\_meta & Dictionary lookup table & \\
\cline{2-5}
...
\cline{2-5}
& ? & uint8[] & cdata & Entropy encoded data stream (see \textsc{Order} / \textsc{RLE} flags) & \\
\cline{2-5}
\multicolumn{6}{|l|}{}\\
\hline
\end{tabular}
...
4 & \textsc{Dict} & map 32-bit values to 8-bit\\
8 & \textsc{X4} & 4-way interleaving of byte streams.\\
...
The Dict and Pack formats have meta data.
This is decoded first (dict then pack as appropriate), before entropy decoding and finally expanding any specified pack and dict transformations in that order.
...
\Function{ArithDecode}{$len$}
  ...
  \EndIf
  \Statex \Comment{Decode meta-data}
  \If{$flags \bitand$ \textsc{Dict}}
    \State $D \gets $\Call{DecodeDictMeta}{}
  \EndIf
  \If{$flags \bitand$ \textsc{Pack}} \Comment{Swap order of this with Dict for consistency?}
  ...
  \Else
    \If{$flags \bitand$ \textsc{Order}}
      \State $data \gets $\Call{DecodeOrder1}{}
    \Else
      \State $data \gets $\Call{DecodeOrder0}{}
    \EndIf
  \EndIf
  \Statex \Comment{Apply data transformations}
  \If{$flags \bitand$ \textsc{Pack}}
    \State $data \gets $\Call{DecodePack}{$data$, $P$}
  \EndIf
  \If{$flags \bitand$ \textsc{Dict}}
    \State $data \gets $\Call{DecodeDict}{$data$, $D$}
  \EndIf
  \State $out \gets data$
  \EndFunction
\end{algorithmic}
...
If present, all other bit flags should be nul, with the possible exception of \textsc{NoSize}.
...
The first meta-dat byte holds $nsym$, the number of distinct values, followed
...
Decoding this meta-data is implemented by the \textsc{DecodePackMeta} function.

\begin{algorithmic}[1]
\Function{DecodePackMeta}{$M$, $nsym$}
  \State $nsym \gets $\Call{ReadByte}{}
...
The meta-data is unpacked at before entropy decoding.
Once the main data block has been decoded, the byte stream is then expanded with the \textsc{DecodePack} function below.

\begin{algorithmic}[1]
\Function{DecodePack}{$len$}
  \If{$nsym <= 1$} \Comment{Constant value}
    \For{$i \gets 0$ to $len-1$}
      \State $out_i \gets M_0$
    \EndFor
  \Statex
  \ElsIf{$nsym <= 2$} \Comment{1 bit per value}
    \For{$i \gets 0$ to $len-1$}
      \If{$i \bmod 8 = 0$}
    ...
    \EndFor
  \Statex
  \ElsIf{$nsym <= 4$} \Comment{2 bits per value}
    \For{$i \gets 0$ to $len-1$}
      \If{$i \bmod 4 = 0$}
        \State $v \gets$ \Call{ReadByte}{}
      \EndIf
    ...
    \EndFor
  \Statex
  \ElsIf{$nsym <= 16$} \Comment{4 bits per value}
    \For{$i \gets 0$ to $len-1$}
      \If{$i \bmod 2 = 0$}
        \State $v \gets$ \Call{ReadByte}{}
      \EndIf
...
\item{\textbf{\textsc{Dict}}:}
% Ideally we'd make DICT and PACK external codecs in their own right.
% Sadly we still need them here too for the name tokeniser, which
% serialises many small compressed streams together in its own format
% to avoid overhead.
...
As with \textsc{Pack}, \textsc{Dict} has disjoint uncompressed
meta-data and compressed data.  The meta-data describes numeric
dictionary.

If our data consists of a few distinct 32-bit values then compression
can be gained by using a lookup table and converting these to 8-bit
values.  We use the term $stride$ to indicate the size of elements in
the input data (4-byte for 32-bit values).
...
\foreach \w/\i [count=\n] in {
  ...
  3/34, 3/12, 3/0, 0/0} {
    ...
    \draw[-] (\n*\x-.5*\x,-0.4)  -- (\n*\x+.5*\x,-0.4);
    \draw[-] (\n*\x-.5*\x, 0.4)  -- (\n*\x+.5*\x, 0.4);
    \node[minimum height=1cm,minimum width=\x,xshift=\n*\x,font=\ttfamily](N\n){\i};
}
\end{tikzpicture}
...
Here we describe the reversal of this; the dictionary decoding method.
In the example above our elements of $V$ were all 2-bytes long
(16-bit).  Currently the \textsc{Dict} format is restricted to a
$stride$ of 4 and element size 2.
The map format itself is described in a meta-data block:
...
\textbf{Bytes} & \textbf{Type} & \textbf{Name} & \textbf{Description}\\
\hline
\sfrac{1}{2}    & nibble(high) & $v$ & Size in bytes for each value in dictionary $V$ (must be 2)\\
\sfrac{1}{2}    & nibble(low)  & $s$ & Stride size in bytes for output (must be 4)\\
...
The first byte indicates the multi-byte stride
size $s$ to map in the low 4 bits and the size $v$ of the multi-byte
values in the top 4 bits, where $s \ge v$.
Currently these must be 4 and 2 respectively.

The next byte holds the number of values $n$ in the map $V$. Following
this is a run-length encoding of the map.  The meta-data stream
consists of a byte holding the number of literals in a row in the top
4 bits and the subsequent number of runs of consecutive literals in a
row in the bottom 4 bits.  For each literal, the next $v$ bytes
(little endian) represent the value.

During decode, these literals will be expressed as $s$-byte little
endian values.  For each run the value is numerically one higher than
before and is not stored.

For example $V = \{1000,1100,1200,1201,1202,1203,1204,2200\}$ is stored in the meta-data stream as:

\begin{tikzpicture}
\draw[-] (0*1cm+.5cm,-0.4) -- (0*1cm+.5cm,+0.4);
\foreach \w/\i [count=\n] in {0/24, 0/08, 0/43, 3/e8, 0/03, 3/4c, 0/04, 3/b0, 0/04, 0/01, 3/98, 0/08} {
    \draw[-] (\n*1cm+.5cm,-0.4) -- (\n*1cm+.5cm,-\w/10);
    \draw[-] (\n*1cm+.5cm,\w/10) -- (\n*1cm+.5cm,0.4);
    \draw[-] (\n*1cm-.5cm,-0.4) -- (\n*1cm+.5cm,-0.4);
    \draw[-] (\n*1cm-.5cm, 0.4) -- (\n*1cm+.5cm, 0.4);
    \node[minimum height=1cm,minimum width=1cm,xshift=\n*1cm,font=\ttfamily](N\n){\i};
}

\node[below=0pt, xshift=0.5cm] at (N1.south west) {$v\ 2$};
\node[below=10pt,xshift=0.5cm] at (N1.south west) {$s\ 4$};

\node[below=0pt, xshift=0.5cm] at (N2.south west) {$n$};

\node[below=0pt, xshift=0.5cm] at (N3.south west) {$run\ 4$};
\node[below=10pt,xshift=0.5cm] at (N3.south west) {$lit\ 3$};

\node[below=1pt, xshift=1cm]   at (N4.south west) {$1000$};
\node[below=1pt, xshift=1cm]   at (N6.south west) {$1100$};
\node[below=1pt, xshift=1cm]   at (N8.south west) {$1200$};

\node[below=0pt, xshift=0.5cm] at (N10.south west) {$run\ 0$};
\node[below=10pt,xshift=0.5cm] at (N10.south west) {$lit\ 1$};

\node[below=1pt, xshift=1cm]   at (N11.south west) {$2200$};
\end{tikzpicture}
...
\Function{DecodeDictMeta}{$len$}
  \State $sv \gets $\Call{ReadByte}{}
  \State $stride \gets sv \bmod 16$ \Comment{Currently must be 4}
  \State $vsize \gets sv \bdiv 16$ \Comment{Currently must be 2}
  ...
  \While{$i < n$}
    ...
    \For{$j \gets 0$ to $lit-1$}
      \State $D_{i+j} \gets $\Call{ReadInt16}{} \Comment{For $vsize = 2$}
    \EndFor
    \State $i \gets i + lit$
    \If{$run > 0$}
...
The compressed data stream follows (either immediately after this or
following the \textsc{Pack} meta-data), encoded according to the other
...
$v$-byte value $V_x$, thus growing the byte stream by $v$ times.

\begin{algorithmic}[1]
\Function{DecodeDict}{$data,\ out\_len$} \Comment{Only for $stride = 4$}
  \State $i \gets 0$
...
\Procedure{DecodeFQZParams}{}
  ...
  \If{$vers \ne 5$}
    \State ERROR
  \EndIf
  \State $gflags \gets$ \Call{ReadByte}{}
  \If{$gflags \bitand 1$} \Comment{$multi\_param$}
    \State $nparam \gets$ \Call{ReadByte}{}
...
%  \State \algalign{have\_qmap}{\gets}   $flags\bitand 1$
  \State \algalign{p.max\_sym}{\gets} \Call{ReadByte}{}
  ...
  \If{$p.flags\bitand 1$} \Comment{Have qmap}
    \For{$i \gets 0$ to $p.max\_sym-1$}
      \State $p.qmap_i \gets$ \Call{ReadByte}{}
    \EndFor
  \EndIf
...
\Procedure{FQZCreateModels}{}
  ...
  \If{$max\_sel > 0$}
    \State $model\_sel \gets $\Call{ModelCreate}{$max\_sel$}
  \EndIf
\EndProcedure
\end{algorithmic}
\end{program}
...
\Function{DecodeFQZNewRecord}{}
  ...
  \Statex
  \If{$param.do\_len$ or $param.first\_len$} \Comment{Decode read length}
...
\Procedure{DecodeFQZ}{}
  ...
  \While{$i < buf\_len$}
    \If{$pos = 0$}\Comment{Reset state at start of each new record}
      \State $x \gets $\Call{DecodeFQZNewRecord}{}
      \If {$is\_dup = 1$}
