...
% Note we have a map for quality (qmap), but not for pos or delta.
%
% Potentially pos map could be useful if we wish to use a context of,
% say, ``cycle 17 was duff but all others are fine''.  We map 100
% cycles to a boolean (is cycle 17 or not) to reduce the context size
% and speed up learning.
%
% We may also wish to have a dedicated qmap used purely for context
% building, rather than also using it, as is now, for collapsing the
% dispersed bins into neighbouring values of 0-N.  E.g. This may be
% useful to distinguish quality of ``is max or not''?
%
% All of these are generalisations of using the division by shifting.
% The map is a lookup table which can do that for us (but slower?).
...
\Procedure{DecodeFQZParams}{}
  ...
  \State \algalign{do\_qmap}{\gets}   $flags\bitand 1$
  ...
  \State \algalign{qmax}{\gets} $2^{qbits}-1$
  \State \algalign{x}{\gets} \Call{ReadByte}{}
  \State \algalign{pshift}{\gets} $x \bmod 16$
  \State \algalign{pbits}{\gets} $x \bdiv 16$
  \State \algalign{x}{\gets} \Call{ReadByte}{}
  \State \algalign{dshift}{\gets} $x \bmod 16$
  \State \algalign{dbits}{\gets} $x \bdiv 16$
  \State \algalign{dmax}{\gets} $2^{dbits}-1$
  \State \algalign{x}{\gets} \Call{ReadByte}{}
  ...
  \State \algalign{dtab}{\gets} \Call{ReadArray}{$2^{dbits}$}
\EndProcedure
\end{algorithmic}
...
%        \State continue\Comment{back to While loop}
  ...
  \Statex
  \State $qctx \gets (qctx << qshift) + q$\Comment{Update context}
  \State $ctx \gets (qctx\bitand qmax) << qloc$
  \If{$pbits > 0$}
    \State $ctx \gets ctx + ($\Call{Min}{$pmax, pos>>pshift$}$ << ploc)$
  \EndIf
  \If{$do\_strand$}
    \State $ctx \gets ctx + (is\_read2 << sloc)$
  \EndIf
  \If{$dbits > 0$}
    \State $dmin \gets$ \Call{Min}{$255, delta>>pshift$}
