...
inbuilt bit-packing and run-length encoding.  The Order-1 rANS Nx16
...
\begin{algorithmic}[1]
\Statex (Reads a list of symbols $A$ used in our alphabet)
...
optionally compressed using the order-0 rANSNx16 codec with a fixed
4-way interleaving.
...
should decode the pack meta-data, the RLE meta-data and the compressed
...
As with the rANSNx16 encoder, the data is rearranged such that every
...
\multicolumn{3}{|r|}{1} & uint8  & $use\_arith$ & Whether compression is arithmetic (1) or rANSNx16 (0)\\
\hline
